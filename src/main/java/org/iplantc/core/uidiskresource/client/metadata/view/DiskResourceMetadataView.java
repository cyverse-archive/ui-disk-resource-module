...
public class DiskResourceMetadataView implements IsWidget {
    ...
    ComboBox<MetadataTemplateInfo> buildTemplateCombo() {
    	templateStore = new ListStore<MetadataTemplateInfo>(new ModelKeyProvider<MetadataTemplateInfo>() {
    	...
    	templateCombo = new ComboBox<MetadataTemplateInfo>(templateStore, new LabelProvider<MetadataTemplateInfo>() {
    	...
    	templateCombo.addValueChangeHandler(new ValueChangeHandler<MetadataTemplateInfo>() {

			@Override
			public void onValueChange(ValueChangeEvent<MetadataTemplateInfo> vce) {
				MetadataTemplateInfo mti = vce.getValue();
    ...
    private FieldLabel buildFieldLabel(IsWidget widget, String lbl, boolean allowBlank) {
    	FieldLabel fl = null;
    	if(!allowBlank) {
    		fl  = new FieldLabel(widget, buildRequiredFieldLabel(lbl));
    	} else {
    		fl = new FieldLabel(widget, lbl);
    	}
    ...
    private TextField buildDateField(boolean allowBlank) {
    	...
    	tf.setEmptyText(DateTimeFormat.PredefinedFormat.DATE_TIME_MEDIUM.toString());
    	tf.addValidator(new Validator<String>() {
			...
			public List<EditorError> validate(Editor<String> editor, String value) {
				try {
					tf.clearInvalid();
					Date d = DateTimeFormat.getFormat( DateTimeFormat.PredefinedFormat.DATE_TIME_MEDIUM).parse(value);
					return null;
				} catch (Exception e) {
					EditorError dee = new DefaultEditorError(editor, "Invalid date / time. Please use " + DateTimeFormat.PredefinedFormat.DATE_TIME_MEDIUM.toString(), value);
    ...
    /**
     * return widget based on type
     *
     * @param label
     * @return
     */
   /* private IsWidget getAttributeValueWidget(String type) {
    ...
    private void initGridEditing() {
    	ContentPanel gridPanel = new ContentPanel(appearance);
    	gridPanel.getHeader().addStyleName(ThemeStyles.getStyle().borderTop());
        alc.add(gridPanel);
        alc.setActiveWidget(gridPanel);
        gridPanel.setHeadingText("User Metadata");
